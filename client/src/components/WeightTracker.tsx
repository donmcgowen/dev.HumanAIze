...
export function WeightTracker() {
  ...
  const addWeightMutation = trpc.weight.addEntry.useMutation({
    onError: (err) => setError(err.message || "Failed to add weight entry"),
  });
  ...
  const handleAddWeight = async () => {
    ...
    try {
      await addWeightMutation.mutateAsync({
        weightLbs: Math.round(Number(weightInput)),
        recordedAt: Date.now(),
  ...
  // Most recent entry = current weight; oldest = starting weight
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (val === "" || /^\d+$/.test(val)) setWeightInput(val);
                }}
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-3">
          ...
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={progressData}>
              ...
              <YAxis stroke="#94a3b8" style={{ fontSize: "12px" }} domain={["dataMin - 5", "dataMax + 5"]} />
              <Tooltip
                contentStyle={{ backgroundColor: "#1e293b", border: "1px solid #475569", borderRadius: "8px" }}
                labelStyle={{ color: "#e2e8f0" }}
                formatter={(value) => [`${value} lbs`, "Weight"]}
              />
              <Line type="monotone" dataKey="weight" stroke="#06b6d4" dot={{ fill: "#06b6d4", r: 4 }} activeDot={{ r: 6 }} strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {entries.length > 0 && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden">
          ...
          >
            ...
            {isLogsExpanded ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
          </button>
          {isLogsExpanded && (
                ...
                {entries.map((entry) => (
                  <div key={entry.id} className="flex items-center justify-between bg-slate-800 p-3 rounded">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-400">{new Date(entry.recordedAt).toLocaleDateString()}</p>
